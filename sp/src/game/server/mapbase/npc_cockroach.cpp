...
class CNPC_Cockroach : public CAI_BaseNPC
{
	DECLARE_CLASS(CNPC_Cockroach, CAI_BaseNPC);
...
LINK_ENTITY_TO_CLASS(npc_cockroach, CNPC_Cockroach);

void CNPC_Cockroach::Spawn()
{
	Precache();

	SetModel("models/npcs/cockroach.mdl");
...
void CNPC_Cockroach::Precache()
{
	PrecacheModel("models/npcs/cockroach.mdl");
...
float CNPC_Cockroach::MaxYawSpeed(void)
{
	return 120.0f;
}

void CNPC_Cockroach::NPCThink(void)
{
...
void CNPC_Cockroach::PickNewDest(int iCondition)
{
	Vector	vecNewDir;
	Vector	vecDest;
	float	flDist;
...
void CNPC_Cockroach::Look(int iDistance)
{
...
void CNPC_Cockroach::Move(float flInterval)
{
...
void CNPC_Cockroach::Touch(CBaseEntity *pOther)
{
	Vector		vecSpot;
	trace_t		tr;
...
void CNPC_Cockroach::Event_Killed(const CTakeDamageInfo &info)
{
...
int CNPC_Cockroach::GetSoundInterests(void)
{
	return	SOUND_CARCASS |
		SOUND_MEAT;
}

void CNPC_Cockroach::Eat(float flFullDuration)
{
	m_flHungryTime = gpGlobals->curtime + flFullDuration;
}

bool CNPC_Cockroach::ShouldEat(void)
{
